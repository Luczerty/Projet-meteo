...
void parcourstemp3 (  pListe  a , FILE* f   )
{
if (a!=NULL) 
{
 
 fprintf (f, "%d %d-%d-%dT%d %f \n", a->numstation ,a->annee ,  a->mois , a->jour  ,  a->heure , a->data );  
parcourstemp3(a->suivant , f);
...
{
    fscanf(t ,"%d %d-%d-%dT%d:%d:%d+%d:%d %f", &numstation , &annee ,&mois , &jour ,&heure ,&poubelle , &poubelle , &poubelle , &poubelle ,&data );
    i++;
    printf("%d %d\n" ,i, numstation);
...
while (z->suivant!=NULL)
{
  ...
  triste->suivant = NULL ;
  z=z->suivant ;
  if (z->numstation!= z->suivant->numstation)
  {
   mergesortdate(&triste);
   /* 
if (r== 1)
{

parcourstemp3 (triste,t2 );
}
else 
{
    reverse2 (&triste);
